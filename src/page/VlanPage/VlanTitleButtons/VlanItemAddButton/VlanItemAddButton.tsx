...
const VlanItemAddButton: React.FC = () => {
  const { isModalVisible, onCloseHandler, onOpenHandler } = useModalVisibleHook()

  const { t } = useTranslation('common')
  return (
    <>
      <AddIconButton onClick={onOpenHandler} tooltipTitle={`${t('create')} VLAN`} bigSize />
      <Modal visible={isModalVisible} onCancel={onCloseHandler} centered mask footer={null}>
        <VlanItemForm title={`${t('create')} VLAN`} onCloseHandler={onCloseHandler} />
      </Modal>
    </>
  )
}

export default VlanItemAddButton
